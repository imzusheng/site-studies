"""Copy the formal A3.43 runtime without mesh simplification.

Usage: python3 scripts/import-a343.py /path/to/luma-remote
...
runtime = source / 'hardware/mechanical/exports/assembly/profiles/industrial_a3_43/web_runtime'
destination = pathlib.Path(__file__).resolve().parents[1] / 'public/models/a343'
manifest = json.loads((runtime / 'ASSEMBLY_MANIFEST.json').read_text())
assert manifest['profile']['id'] == 'industrial_a3_43'
assert manifest['sceneKind'] == 'physical-assembly-validation'
assert len(manifest['parts']) == 471
...
manifest['assetBase'] = '/models/a343/'
...
print(f"A3.43: {len(manifest['parts'])} objects, {raw_bytes:,} → {compressed_bytes:,} bytes (lossless gzip)")
