...
test('Visible rim can contact the ball edge',()=>assert(collide(new V(R.RACKET.rx+R.BALL_R*.45,0,-.2),new V(R.RACKET.rx+R.BALL_R*.45,0,.2)).hit));
...
test('Contact stores the interpolated surface position',()=>{const c=collide(new V(.06,.08,-1),new V(.06,.08,1));assert(c.hit&&c.gap<=R.BALL_R+.012);close(c.u,.06/R.RACKET.rx);close(c.v,.08/R.RACKET.ry);});
...
test('Bounce reverses downward motion',()=>{const b=new Ball();b.p.y=R.BALL_R+.001;b.v.y=-4;assert(R.integrate(b,1/120));assert(b.v.y>0&&b.bounces===1);});
...
test('Topspin produces a stronger forward / vertical rebound',()=>{const a=new Ball(),b=new Ball();a.p.y=b.p.y=R.BALL_R+.001;a.v.y=b.v.y=-4;a.v.z=b.v.z=12;a.top=2400;b.top=-750;R.integrate(a,1/120);R.integrate(b,1/120);assert(a.v.y>b.v.y&&a.v.z>b.v.z);});
...
for(const [name,z,off,lead] of [['Perfect',-.52,.2,0],['Good',-.67,.4,0],['Early',-.52,.3,.10],['Late',-.52,.3,-.12]])test('Timing tier: '+name,()=>assert.equal(R.timingFor(z,off,lead),name));
...
test('Planned grip uses the exact handle offset rather than a separate swing path',()=>{const a=new Actor(R.C.lime);a.start(new V(.7,1.1,7.4),S);a.tick(.14,S);const grip=new V().copy(a.racket.p).addScaled(a.racket.v,-R.RACKET.grip);close(grip.dist(a.grip),0);});
...
test('Charged swing has an outside backswing, forward contact and cross-body finish',()=>{for(const hand of [1,-1]){const a=new Actor(R.C.lime);a.hand=hand;a.charging=true;for(let i=0;i<85;i++)a.tick(1/120,S);const prep=a.racket.p.x;assert(a.racket.p.z>a.z-.35);a.start(new V(hand*.73,1.25,7.6),S);a.swing=a.windup;a.pose(.01,S);close(a.racket.p.z,7.6);a.swing=a.windup+.36;a.pose(.01,S);assert(a.racket.p.x*hand<0);assert(Math.abs(prep-a.racket.p.x)>.65);}});
test('A complete planned swing occupies load/contact/follow/recovery phases',()=>{const a=new Actor(R.C.lime);a.start(new V(.73,1.25,7.4),S);const stages=new Set;for(let i=0;i<130;i++){a.tick(1/120,S);stages.add(a.actionPhase);}assert([...stages].some(x=>x.includes('DROP')));assert([...stages].some(x=>x.includes('DRIVE')));assert([...stages].some(x=>x.includes('FOLLOW')));assert([...stages].some(x=>x.includes('RECOVER')));assert.equal(a.swing,-1);});
