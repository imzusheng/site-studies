...
for who in ['rain','snow']:
 ...
 bn={b.name:b for b in arm.data.bones}
 def bone(*candidates):
  for n in candidates:
 ...
 def classify(name,position=None):
  ...
  if any(x in s for x in ['thumb','index','middle','ring','pinky','hand']):return 7 if side>0 else 11
 ...
 def material(m):
  ...
  if 'body' in name:
   ps=[images.get(f'TEX-rain_body_diffuse.{1001+i}.png') for i in range(3)]
   if who=='rain' and all(ps):tex=texture(ps,True);atlas=True;col=[1,1,1,1]
   else:
 ...
 meshes=[o for o in bpy.context.scene.objects if o.type=='MESH' and not o.hide_render and o.name.startswith('GEO-') and not any(w in o.name.lower() for w in ['scarf','cornea','eye_dots']) and any(m.type=='ARMATURE' for m in o.modifiers)]
 outstats=[]
 for o in meshes:
  o.hide_set(False)
  ...
  groups={g.index:g.name for g in o.vertex_groups};ws=[];coords=[]
  ...
  coords=np.asarray(coords);faces=np.array([list(t.vertices) for t in me.loop_triangles]);norm=np.zeros_like(coords);fn=np.cross(coords[faces[:,1]]-coords[faces[:,0]],coords[faces[:,2]]-coords[faces[:,0]])
  ...
  for mi in sorted(set(t.material_index for t in me.loop_triangles)):
   m=me.materials[mi] if me.materials else None
   if not m:continue
   ...
   for t in me.loop_triangles:
    ...
    for vi,li in zip(t.vertices,t.loops):
     uv=list(me.uv_layers.active.data[li].uv) if me.uv_layers.active else [0,0]
     if atlas:uv[0]/=3
