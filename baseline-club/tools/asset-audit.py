...
for who in ['rain','snow']:
 ...
 for m in bpy.data.materials:
  ...
  if m.node_tree:
   for n in m.node_tree.nodes:
    nodes.append({'name':n.name,'type':n.type,'image':n.image.name if hasattr(n,'image') and n.image else None,'group':n.node_tree.name if n.type=='GROUP' and n.node_tree else None,'inputs':{i.name:list(i.default_value) if hasattr(i.default_value,'__len__') else i.default_value for i in n.inputs if hasattr(i,'default_value') and not i.is_linked and isinstance(i.default_value,(int,float,bpy.types.bpy_prop_array))}})
 ...
 # Restore rest pose; only artist render meshes and deformation rig are exported.
 def unhide(layer):
