"""Export Rain v3 original weights and evaluated body corrective rig for the club.
Run with Blender 5.2 --python-use-system-env -b <rain.blend> --python this.py.
...
TEX=ROOT/'.cache/rain-v3/Rain v3.3/textures'
S=1.1084452851618694
C=Matrix(((S,0,0,0),(0,0,S,0),(0,-S,0,0),(0,0,0,1)))
CR=C.to_3x3().normalized()
old=ROOT/'.cache/rain-original-18.glb'
if not old.exists():old.write_bytes((ROOT/'assets/rain.glb').read_bytes())
...
props=arm.pose.bones['Properties_IKFK']
for k in list(props.keys()):
 if k.startswith('ik_') and isinstance(props[k],(int,float)):props[k]=0.0
for pb in arm.pose.bones:pb.matrix_basis=Matrix.Identity(4)
arm.update_tag();bpy.context.view_layer.update()
mapping={0:['MSTR-Pelvis'],1:['FK-Spine','FK-RibCage','FK-Chest'],2:['FK-Neck'],3:['FK-Head'],4:['MSTR-Clavicle.L'],5:['FK-Upperarm.L'],6:['FK-Forearm.L'],7:['FK-Hand.L'],8:['MSTR-Clavicle.R'],9:['FK-Upperarm.R'],10:['FK-Forearm.R'],11:['FK-Hand.R'],12:['FK-Thigh.L'],13:['FK-Shin.L'],14:['FK-Foot.L'],15:['FK-Thigh.R'],16:['FK-Shin.R'],17:['FK-Foot.R']}
targets={}
...
pelvis=arm.pose.bones['MSTR-Pelvis'];loc=bpy.data.objects.new('MOCAP-pelvis-position',None);bpy.context.collection.objects.link(loc);loc.location=pelvis.bone.head_local
 ...
 for side,amount in [('R',1.0),('L',.28)]:
  for finger in ['Index','Middle','Ring','Pinky']:
   for j,ang in enumerate([.72,1.05,.70],1):
    pb=arm.pose.bones[f'FK-{finger}{j}.{side}'];pb.rotation_mode='XYZ';pb.rotation_euler.x=ang*amount
  for j,ang in enumerate([.28,.45,.5],1):
   pb=arm.pose.bones[f'FK-Thumb{j}.{side}'];pb.rotation_mode='XYZ';pb.rotation_euler.x=ang*amount
grip();bpy.context.view_layer.update()
# The authoring rest pose has a horizontal ponytail. Retain its four native
# deformation bones and pose a natural downward arc instead of welding it to Head.
for j,angle in enumerate([.12,.47,.43,.20],1):
 pb=arm.pose.bones[f'FK-Hair_Ponytail{j}'];r=pb.bone.matrix_local.to_quaternion();pb.rotation_mode='QUATERNION';pb.rotation_quaternion=r.inverted()@Quaternion(Vector((1,0,0)),-angle)@r
bpy.context.view_layer.update()
def parent_index(name):
 side=1 if name.endswith('.R') else 0
 if any(k in name for k in ['Index','Middle','Ring','Pinky','Thumb','Hand']):return 11 if side else 7
 if 'Forearm' in name:return 10 if side else 6
 if 'Upperarm' in name:return 9 if side else 5
 if 'Clavicle' in name:return 8 if side else 4
 if 'Shin' in name:return 16 if side else 13
 ...
 if name=='DEF-Spine1' or 'Pelvis' in name:return 0
 if 'Spine' in name:return 1
 return 3
body_terms=['Spine','Pelvis','Neck','Clavicle','Upperarm','Forearm','Hand','Thumb','Index','Middle','Ring','Pinky','Thigh','Shin','Foot','Toe','Hair']
def mapped(name):
...
mesh_names=['body','head','top','jeans','shoes','eyes','eyebrows','eyelashes','hair_main','hair_ponytail','hair_strand','hairband','gums_upper','gums_lower','tongue']
objects=[bpy.data.objects['GEO-rain-'+s] for s in mesh_names]
used=set(['DEF-Head'])
for ob in objects:
...
def local_parts(mat):
 p,q,s=mat.decompose();return [round(x,7) for x in p],[round(q.x,7),round(q.y,7),round(q.z,7),round(q.w,7)],[round(x,7) for x in s]
def source_pose(c,f):
...
def apply_source(qs,p,kind=None,sample_time=None):
 ...
 # torso after adapting adult mocap to Rain's shorter arms. No runtime chasing.
 torso=C@arm.pose.bones['DEF-Spine2'].matrix
 inv=torso.inverted();orientation=torso.to_quaternion()
 for side in ['R','L']:
  sn,en,hn=[f'FK-{x}.{side}' for x in ['Upperarm','Forearm','Hand']]
  s,e,h=[C@arm.pose.bones[n].matrix.translation for n in [sn,en,hn]]
  hand_center=C@(arm.pose.bones[f'DEF-Hand.{side}'].matrix@Vector((0,.075,-.016)))
 ...
 if kind=='forehand' and sample_time is not None and sample_time<.22:
  u=min(1,max(0,(sample_time-.08)/.14));clearance_weight=1-u*u*(3-2*u)
 if clearance_weight>0:
  # During the recorded serve wind-up, the adapted racket can cross the neck.
  # Turn the actual wrist (and all fingers) just enough to leave the head cone.
  # This is evaluated offline and leaves the racket rigidly attached to the grip.
  for bone,offset,radius in [('DEF-Head',.09,.235),('DEF-Neck',.035,.19),('DEF-Spine2',.08,.34)]:
   hand=C@arm.pose.bones['DEF-Hand.R'].matrix;grip=hand@Vector((-.028,.079,0));axis=(hand.to_3x3()@Vector((0,0,1))).normalized()
   ...
   if angle<limit:
    ...
    target=targets['FK-Hand.R'][1];target.rotation_quaternion=(CR.inverted().to_quaternion()@dq@CR.to_quaternion())@target.rotation_quaternion
    bpy.context.view_layer.update()
 return mats
def bake():
 clips={};audit={}
 ...
 (ROOT/'assets/rain-motions.json').write_text(json.dumps({'version':1,'names':names,'parents':parents,'source':'Blender Studio Rain v3 native deformation rig, FK retarget of six recorded tennis clips, with phalange grip','clips':clips},separators=(',',':')),encoding='utf8')
 return clips
...
def export_glb():
 ...
 gl={'asset':{'version':'2.0','generator':'Baseline Club Rain native-deform adapter'},'scene':0,'scenes':[{'nodes':[0]}],'nodes':[{'name':'Rain','children':[],'extras':{'rig':rig,'professionalRig':{'names':names,'parents':parents,'rightHand':'DEF-Hand.R','source':'Blender Studio Rain v3','fingerGrip':True},'artist':'Blender Studio'}}],'meshes':[],'skins':[],'accessors':[],'bufferViews':[],'buffers':[],'materials':[],'textures':[],'images':[],'samplers':[{'magFilter':9729,'minFilter':9987,'wrapS':10497,'wrapT':10497}]}
 blob=bytearray()
 ...
 def texture(name,atlas=False):
  ...
  if atlas:
   ...
   for i in range(3):im.paste(Image.open(TEX/f'TEX-rain_body_diffuse.{1001+i}.png').convert('RGB').resize((1024,1024)),(i*1024,0))
  else:
 ...
 def material(m):
  ...
  key=name.split('.')[-1];colors={'body':[1,1,1,1],'top':[.79,.83,.71,1],'jeans':[.22,.34,.27,1],'shoes':[.72,.74,.64,1],'hair':[.32,.2,.10,1],'eyebrows':[.065,.035,.018,1],'eyelashes':[.025,.017,.01,1],'gums':[.6,.25,.22,1],'teeth':[.9,.86,.74,1],'tongue':[.64,.23,.19,1],'hairband':[.42,.14,.09,1],'metal':[.5,.5,.45,1],'socks':[.8,.82,.72,1],'laces':[.76,.77,.69,1],'eyes':[1,1,1,1]}
  ...
  if key=='eyes':pbr['baseColorTexture']={'index':texture('TEX-rain_eyes.png')}
  if key=='hair':pbr['baseColorTexture']={'index':texture('TEX-rain_hair_diffuse.png')};pbr['baseColorFactor']=[1,1,1,1]
  if key=='jeans':pbr['baseColorTexture']={'index':texture('TEX-rain_jeans_diffuse.png')};pbr['baseColorFactor']=[.7,.82,.72,1]
 ...
 for ob in objects:
  ...
  if ob.animation_data:
   for driver in ob.animation_data.drivers:
    if 'modifiers[' in driver.data_path and driver.data_path.endswith('.levels'):driver.mute=True
  bpy.context.scene.render.use_simplify=False
  for mod in ob.modifiers:
   if mod.type in ['ARMATURE','LATTICE','CORRECTIVE_SMOOTH']:mod.show_viewport=False
   elif mod.type=='SUBSURF':mod.levels=0 if any(k in ob.name for k in ['jeans','shoes','gums','tongue']) else 1;mod.show_viewport=True
  ...
  for mi in sorted(set(t.material_index for t in me.loop_triangles)):
   pos=[];normal=[];uv=[];js=[];ws=[]
   mat=ob.data.materials[mi] if mi<len(ob.data.materials) else None
   for tri in me.loop_triangles:
    if tri.material_index!=mi:continue
    for li in tri.loops:
     vi=me.loops[li].vertex_index;v=me.vertices[vi];pos.append(list(C@(ob.matrix_world@v.co)));normal.append(list(CR@(ob.matrix_world.to_3x3()@me.corner_normals[li].vector)));u=list(me.uv_layers.active.data[li].uv) if me.uv_layers.active else [0,0]
     if mat and mat.name=='MAT-rain.body':u[0]/=3
 ...
 (ROOT/'assets/rain-pro.glb').write_bytes(out)
 (ROOT/'.cache/rain-pro-audit.json').write_text(json.dumps({'bones':len(names),'triangles':sum(gl['accessors'][p['indices']]['count']//3 for m in gl['meshes'] for p in m['primitives']),'bytes':len(out),'sourceRigBones':len(arm.data.bones),'meshes':len(objects),'materials':len(materials),'textures':len(texcache)},indent=2))
